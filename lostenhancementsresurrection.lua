--[[
╔══════════════════════════════════════════════════════════════════╗
║              lostenhancementsresurrection RESUMMON — Universal Keyless Hub               ║
...
-- ═══════════════════════════════════════════════════════════════
if _G.lostenhancementsresurrectionRunning then
    _G.lostenhancementsresurrectionRunning = false
    wait(0.15)
end
if _G.lostenhancementsresurrectionConnections then
    for _, c in ipairs(_G.lostenhancementsresurrectionConnections) do pcall(function() c:Disconnect() end) end
end
if _G.lostenhancementsresurrectionDrawings then
    for _, d in ipairs(_G.lostenhancementsresurrectionDrawings) do pcall(function() d:Remove() end) end
end
if _G.lostenhancementsresurrectionHighlights then
    for _, h in ipairs(_G.lostenhancementsresurrectionHighlights) do pcall(function() h:Destroy() end) end
end
for _, c in ipairs(game:GetService("CoreGui"):GetChildren()) do
    if c.Name == "ui" or c.Name:find("lostenhancementsresurrection") then pcall(function() c:Destroy() end) end
end

_G.lostenhancementsresurrectionRunning     = true
_G.lostenhancementsresurrectionConnections = {}
_G.lostenhancementsresurrectionDrawings    = {}
_G.lostenhancementsresurrectionHighlights  = {}
...
local function nD(t, p) local d = Drawing.new(t); for k,v in pairs(p or {}) do d[k]=v end; table.insert(_G.lostenhancementsresurrectionDrawings, d); return d end
...
local function mkESP(p)
    ...
    sb.Parent = workspace
    d.SB = sb
    table.insert(_G.lostenhancementsresurrectionHighlights, sb)
    ESPObjects[p] = d
end
...
local function enableNoclip()
    ...
    table.insert(_G.lostenhancementsresurrectionConnections, noclipConn)
end
...
local function toggleSnow(on)
    ...
    snowGui.Name = "lostenhancementsresurrectionSnow"
...
local function setupAntiFling()
    ...
    bv.Parent   = hrp
    _G.lostenhancementsresurrectionAntiFlingBV = bv
end
local function disableAntiFling()
    if _G.lostenhancementsresurrectionAntiFlingBV then
        pcall(function() _G.lostenhancementsresurrectionAntiFlingBV:Destroy() end)
        _G.lostenhancementsresurrectionAntiFlingBV = nil
    end
end
...
local mainConn = RunService.RenderStepped:Connect(function(dt)
    if not _G.lostenhancementsresurrectionRunning then return end
...
table.insert(_G.lostenhancementsresurrectionConnections, mainConn)
...
table.insert(_G.lostenhancementsresurrectionConnections, jumpConn)
...
table.insert(_G.lostenhancementsresurrectionConnections, moveConn)
...
table.insert(_G.lostenhancementsresurrectionConnections, afkConn)

-- Player cleanup
table.insert(_G.lostenhancementsresurrectionConnections, Players.PlayerRemoving:Connect(rmESP))
...
local Window  = VapeLib:Window("lostenhancementsresurrection Resummon", Color3.fromRGB(0, 200, 255), Enum.KeyCode.RightShift)
...
MoveTab:Button("Save Teleport Position", function()
    ...
    if root then
        ...
        print("[lostenhancementsresurrection] TP position saved:", root.Position)
    end
end)
...
MoveTab:Button("Teleport Back to Saved Position", function()
    if Cfg.TpBackPos then
        ...
        print("[lostenhancementsresurrection] No saved position!")
    end
end)
...
UtilTab:Button("Copy JobId to Clipboard", function()
    ...
    if setclipboard then
        setclipboard(id)
        print("[lostenhancementsresurrection] Copied JobId:", id)
    else
        print("[lostenhancementsresurrection] JobId:", id)
    end
end)
...
UtilTab:Button("Server Hop (Smallest Server)", function()
    ...
    local ok, result = pcall(function()
        ...
        if #servers > 0 then
            ...
            print("[lostenhancementsresurrection] No other servers found.")
        end
    end)
    if not ok then print("[lostenhancementsresurrection] Server hop error:", result) end
end)
...
UtilTab:Button("Save Config to Clipboard", function()
    ...
    print("[lostenhancementsresurrection] Config saved to clipboard!")
end)
...
SetTab:Button("Reset All Movement", function()
    ...
    print("[lostenhancementsresurrection] Movement reset.")
end)

SetTab:Button("Unload lostenhancementsresurrection Resummon", function()
    _G.lostenhancementsresurrectionRunning = false
    for _, c in ipairs(_G.lostenhancementsresurrectionConnections) do pcall(function() c:Disconnect() end) end
    for _, d in ipairs(_G.lostenhancementsresurrectionDrawings) do pcall(function() d:Remove() end) end
    for _, h in ipairs(_G.lostenhancementsresurrectionHighlights) do pcall(function() h:Destroy() end) end
    ...
    for _, c in ipairs(CoreGui:GetChildren()) do
        if c.Name == "ui" or c.Name == "lostenhancementsresurrectionSnow" then pcall(function() c:Destroy() end) end
    end
